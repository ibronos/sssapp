...
const Index: React.FC = () => {
    ...
    const loadData = async () => {
        ...
        try {
            await axios.get(`${import.meta.env.VITE_SERVER_HOST}/izins`)
            .then(
                response => {
    ...
    const handleDelete = async (id: number) => {
      if (confirm(`Are you sure want to delete ${id}?`) == true) {
          await axios.delete(`${import.meta.env.VITE_SERVER_HOST}/izin/${id}`)
          .then(() => {
    ...
    return (
      <>
        ...
        <button onClick={handleEdit} className="flex justify-center rounded bg-primary px-6 py-2 font-medium text-gray hover:bg-opacity-90 mb-4 ml-auto" type="submit">
          Add {pageName}
        </button>
            ...
            <tbody>
              {items.map((item:any, index) => (
                <tr key={index}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">
                    {item.izin_jenis.name}
                    </p>
                  </td>

                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">
                    {item.izin_approve.name}
                    </p>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">
                    {parseInt(item.is_canceled) == 1 ? "Ya" : "Tidak"}
                    </p>
                  </td>
